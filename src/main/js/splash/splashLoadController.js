...
define([
	...
	function load(onLoadDone){
		let assetPack = window.location.search.match(/&?assetPack=(\w+)&?/)[1];
		let languageCode = window.location.search.match(/&?language=([^&]+)&?/)[1];
		let skinCode = window.location.search.match(/&?skincode=(\w+)&?/)[1];
		ResourceLoader.initDefault('assetPacks/'+assetPack, languageCode, skinCode);
		let defaultLoader = ResourceLoader.getDefault();
		let i18nSubLoader = new TextFilesSubLoader({type:'i18n', parseJson:true});
		i18nSubLoader.origLoad = i18nSubLoader.load;
		i18nSubLoader.load = function(fileMap, options){
			for(let url in fileMap){
